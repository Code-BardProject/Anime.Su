...
import siteConfiguration from './src/utils/key/install/site.json'

// Vite config — https://vitejs.dev/config/
export default defineConfig(({ mode }) => {
  // The install/deploy-preview script passes `--mode development` for cached-preview builds.
  ...
  return {
    ...
    server: {
      ...
      proxy: {
        '/api': {
          target: (process.env.VITE_API_PROXY_TARGET || process.env.VITE_API_URL || 'http://localhost:5004').replace(/\/api\/?$/, ''),
          changeOrigin: true,
        },
        '/uploads': {
          target: (process.env.VITE_API_PROXY_TARGET || process.env.VITE_API_URL || 'http://localhost:5004').replace(/\/api\/?$/, ''),
      ...
      watch: { ignored: ['**/src/utils/key/install/**'] },
    },
    preview: {
      host: '0.0.0.0',
      port: parseInt(process.env.PORT || '8443'),
